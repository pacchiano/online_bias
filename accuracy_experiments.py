...
def run_and_plot(
    ...
    test_biased_accuracies_cum_averages_summary = np.zeros(
        (num_experiments, int(max_num_steps / logging_frequency))
    )
    ...
    plt.plot(
        ...
        label="Online Decision Test - no optimism",
        linestyle="dashed",
        linewidth=3.5,
        color="blue",
    )
    plt.fill_between(
        timesteps,
        mean_test_biased_accuracies_cum_averages
        - 0.5 * std_test_biased_accuracies_cum_averages,
        mean_test_biased_accuracies_cum_averages
        + 0.5 * std_test_biased_accuracies_cum_averages,
        color="blue",
        alpha=0.1,
    ...
    plt.fill_between(
        timesteps,
        mean_accuracies_cum_averages - 0.5 * std_accuracies_cum_averages,
        mean_accuracies_cum_averages + 0.5 * std_accuracies_cum_averages,
        color="red",
        alpha=0.1,
    )
    ...
    plt.fill_between(
        timesteps,
        mean_train_biased_accuracies_cum_averages
        + 0.5 * std_train_biased_accuracies_cum_averages,
        mean_train_biased_accuracies_cum_averages
        - 0.5 * std_train_biased_accuracies_cum_averages,
        color="violet",
        alpha=0.1,
    ...
    if epsilon_greedy:
        plt.title(
            "Test and Train Accuracies {} - Epsilon Greedy {}".format(dataset, epsilon)
        )
        plot_name = "{}_test_train_accuracies_epsgreedy_{}".format(dataset, epsilon)
    if adjust_mahalanobis:
        plt.title(
            "Test and Train Accuracies {} - Optimism alpha {} - Mreg {}".format(
                dataset, alpha, mahalanobis_regularizer
            )
        )
        plot_name = "{}_test_train_accuracies_optimism_alpha_{}_mahreg_{}".format(
            dataset, alpha, mahalanobis_regularizer
        )
    ...
    plt.fill_between(
        timesteps,
        mean_train_cum_regret_averages - 0.5 * std_train_cum_regret_averages,
        mean_train_cum_regret_averages + 0.5 * std_train_cum_regret_averages,
        color="blue",
        alpha=0.1,
    )
    ...
    pickle.dump(
        ...
        open(
            "./experiment_results/data/T{}/{}.p".format(max_num_steps, plot_name), "wb"
        ),
    )
...
def main():
    ...
    for alpha in [1, 2, 3, 4, 5, 10]:
        ...
        epsilon_greedy = False
        for mahalanobis_regularizer in [0.1, 1]:  # [.1, 1]:
            run_and_plot(
                dataset,
                logging_frequency,
                max_num_steps,
