...
def run_experiment( dataset, logging_frequency, max_num_steps, logistic_learning_rate, threshold, 
  biased_threshold, batch_size, mahalanobis_regularizer, adjust_mahalanobis,epsilon_greedy, epsilon, alpha ):
  ...
  while counter < max_num_steps:
    ...
    for i in range(len(global_biased_prediction)):
      ...
      if global_biased_prediction[i] > biased_threshold or (epsilon_greedy and np.random.random() < epsilon):
    ...
    if counter % logging_frequency == 0:
      ...
      protected_accuracies_list = [protected_accuracies_list[i] + [ protected_accuracies[i] ] for i in range(len(PROTECTED_GROUPS))]
      plt.figure(figsize=(18,16))
      plt.ylim(.2, 1)
      plt.plot(accuracies_list, label = "Global", linewidth = 3.5, color = "blue")
      [plt.plot(protected_accuracies_list[i], label = PROTECTED_GROUPS[i], linewidth = 3.5, color = colors[i]) for i in range(len(PROTECTED_GROUPS))]
      ...
      biased_protected_accuracies_list = [biased_protected_accuracies_list[i] + [ biased_protected_accuracies[i] ] for i in range(len(PROTECTED_GROUPS))]
      
      plt.plot(biased_accuracies_list, label = "Global Biased", linestyle = "dashed", linewidth = 3.5, color="blue")
      [plt.plot(biased_protected_accuracies_list[i], label = "Biased {}".format(PROTECTED_GROUPS[i]), linestyle = "dashed", linewidth = 3.5, color = colors[i]) for i in range(len(PROTECTED_GROUPS))]
      ...
      for i in range(len(PROTECTED_GROUPS)):
          ...
          print("                                                               Accuracy {} ".format(PROTECTED_GROUPS[i]), protected_accuracy)
          plt.subplot(1,len(PROTECTED_GROUPS), i+1)        

          _, _, _ = plt.hist( global_probabilities_list, 50, density=True, facecolor='g', alpha=0.75 , label = "global"  )
          plt.xlabel('ys')
          plt.ylabel('Probability')
          plt.title('Histogram of responses {}.'.format(protected_group))
          _, _, _= plt.hist( probabilities_list, 50, density=True, facecolor='r', alpha=0.75 , label = protected_group)
      ...
      for i in range(len(PROTECTED_GROUPS)):
          ...
          print("                                                              Biased Accuracy {} ".format(PROTECTED_GROUPS[i]), biased_protected_accuracy)
          plt.subplot(1,len(PROTECTED_GROUPS), i+1)        

          _, _, _ = plt.hist( biased_global_probabilities_list, 50, density=True, facecolor='g', alpha=0.75 , label = "global", color = 'orange'  )
          plt.xlabel('ys')
          plt.ylabel('Probability')
          plt.title('Biased Histogram of responses {}.'.format(protected_group))
          _, _, _= plt.hist( biased_probabilities_list, 50, density=True, facecolor='r', alpha=0.75 , label = protected_group, color = "blue")
      ...
      if dataset == "SVM" or dataset == "MultiSVM":
        print("Groups ", PROTECTED_GROUPS, " Probabilities ",  probabilities , " Variances ",    variances, " Class list ", class_list_per_center )
        if d == 2:
          plt.figure(figsize=(24,5))
          plt.subplot(1, 2, 1)
          plt.title("Datapoints and classifier")
          test_dataset.plot(batch_size = 1000, model = model, names = PROTECTED_GROUPS)
          plt.subplot(1,2,2)
          plt.title("Biased datapoints and classifier")
          test_dataset.plot(batch_size = 1000, model = model_biased, names = PROTECTED_GROUPS)
          plt.show()