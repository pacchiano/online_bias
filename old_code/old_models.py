...
class TorchBinaryLogisticRegression:
    def __init__(self, random_init = False, fit_intercept = True, dim = None, alpha = 1, MLP = True, hidden_size = 10):
...
class BinaryLogisticRegression:
    ...
    def predict_prob(self, batch_X, inverse_data_covariance = []):
      ...
      if len(inverse_data_covariance) == 0:     
        ...
        ### adjust this using the inverse_data_covariance
        ### for each datapoint return the max 
        return self.__sigmoid(np.dot(batch_X, self.theta) + self.alpha*self.__inverse_covariance_norm(batch_X, inverse_data_covariance))
